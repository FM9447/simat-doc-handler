...
const seedDefaultDutyCategories = async () => {
  try {
    ...
    console.log('✅ Auto-seeded Duty Categories into Drive DB');
  } catch (err) {
    ...
    const initialFlows = [
      {
        ...
        elements: [
          { id: 'el_1', kind: 'field', label: 'Duty Category / Club', type: 'select', options: ['IEDC (Innovation & Entrepreneurship Cell)', 'NSS (National Service Scheme)', 'MuLearn Campus Chapter', 'IEEE Student Branch', 'College Sports Team', 'Arts & Cultural Event', 'Department Technical Fest', 'Other Official Representation'], required: true, visible: true },
          { id: 'el_2', kind: 'field', label: 'Event or Activity Name', type: 'text', required: true, visible: true, placeholder: 'e.g. State Hackathon / Annual NSS Camp' },
          { id: 'el_3', kind: 'field', label: 'Duty Leave Schedule', type: 'table', required: true, visible: true, placeholder: 'Tabular Schedule of Dates & Hours' },
          { id: 'el_4', kind: 'field', label: 'Total Hours Granted', type: 'text', required: true, visible: true, placeholder: 'e.g. 12 Hours / 4 Periods' },
        ],
      },
      {
        ...
        elements: [
          { id: 'el_1', kind: 'field', label: 'Academic Year', type: 'text', required: true, visible: true, placeholder: 'e.g. 2025-2026', hint: 'Current academic year' },
          { id: 'el_2', kind: 'field', label: 'Course', type: 'select', options: ['B.Tech Computer Science & Engineering', 'B.Tech Electronics & Communication', 'B.Tech Electrical & Electronics', 'B.Tech Mechanical Engineering', 'B.Tech Civil Engineering', 'M.Tech', 'Diploma'], required: true, visible: true },
          { id: 'el_3', kind: 'field', label: 'Semester', type: 'select', options: ['Semester 1', 'Semester 2', 'Semester 3', 'Semester 4', 'Semester 5', 'Semester 6', 'Semester 7', 'Semester 8'], required: true, visible: true },
          { id: 'el_4', kind: 'field', label: 'Purpose of Certificate', type: 'select', options: ['Passport Application', 'Bank Education Loan', 'Bus / Railway Concession', 'Industrial Internship / Training', 'Scholarship Application', 'External Competition / Hackathon', 'Other Official Purpose'], required: true, visible: true },
        ],
      },
      {
        ...
        elements: [
          { id: 'el_1', kind: 'field', label: 'Admission No or KTU Reg No', type: 'text', required: true, visible: true, placeholder: 'e.g. ADM2021-045 / SPT21CS001' },
          { id: 'el_2', kind: 'field', label: 'Course', type: 'text', required: true, visible: true, placeholder: 'e.g. B.Tech Computer Science & Engineering' },
          { id: 'el_3', kind: 'field', label: 'Date of Birth', type: 'date', required: true, visible: true },
          { id: 'el_4', kind: 'field', label: 'Admission Date', type: 'date', required: true, visible: true },
          { id: 'el_5', kind: 'field', label: 'Leaving Date', type: 'date', required: true, visible: true },
          { id: 'el_6', kind: 'field', label: 'Reason for Leaving', type: 'select', options: ['Course Completed', 'Relocation / Migration', 'Higher Studies Admission', 'Personal Reasons'], required: true, visible: true },
          { id: 'el_7', kind: 'field', label: 'Promotion Status', type: 'select', options: ['Qualified for Promotion to Next Semester', 'Course Successfully Completed', 'N/A'], required: true, visible: true },
          { id: 'el_8', kind: 'field', label: 'Character and Conduct', type: 'select', options: ['Exemplary', 'Very Good', 'Good'], required: true, visible: true },
        ],
      },
      {
        name: 'NOC',
        ...
        elements: [
          { id: 'el_1', kind: 'field', label: 'Event or Internship Title', type: 'text', required: true, visible: true, placeholder: 'e.g. Industrial Internship / Workshop on AI' },
          { id: 'el_2', kind: 'field', label: 'Company or Institution', type: 'text', required: true, visible: true, placeholder: 'e.g. KSEB / Keltron / ISRO / Tech Firm' },
          { id: 'el_3', kind: 'field', label: 'Location', type: 'text', required: true, visible: true, placeholder: 'e.g. Trivandrum / Kochi' },
          { id: 'el_4', kind: 'field', label: 'Semester', type: 'select', options: ['Semester 1', 'Semester 2', 'Semester 3', 'Semester 4', 'Semester 5', 'Semester 6', 'Semester 7', 'Semester 8'], required: true, visible: true },
          { id: 'el_5', kind: 'field', label: 'Start Date', type: 'date', required: true, visible: true },
          { id: 'el_6', kind: 'field', label: 'End Date', type: 'date', required: true, visible: true },
        ],
      },
      {
        ...
        elements: [
          { id: 'el_1', kind: 'field', label: 'Duration of Study', type: 'text', required: true, visible: true, placeholder: 'e.g. August 2021 to May 2025' },
          { id: 'el_2', kind: 'field', label: 'Overall Conduct', type: 'select', options: ['Exemplary', 'Very Good', 'Good'], required: true, visible: true },
        ],
      },
    ];

    for (const flow of initialFlows) {
      await DocumentType.findOneAndUpdate({ name: flow.name }, flow, { upsert: true, new: true });
    }

    console.log('✅ Auto-seeded KTU document types into Drive DB');
  } catch (err) {
...
const connectDB = async () => {
  try {
    ...
    console.log('Drive DB initialization complete');
  } catch (error) {
    console.error(`Drive DB init error: ${error.message}`);
    process.exit(1);
  }
};
